...
import PrivacyDropdown from "../button/PrivacyDropdown";
...
// interface Image {
//   file: File; // We only need the File object
//   url: string;
// }
...
const PostModal = () => {
  ...
  const [images, setImages] = useState<File[]>([]);  // Store selected files here
  const [imageUrls, setImageUrls] = useState<string[]>([]);  // Store image URLs here
  const [addPost] = useAddPostMutation();
  const { userId } = useUser();  // Assuming this returns userId

  useEffect(() => {
    // Generate object URLs for the selected files
    const urls = images.map((image: File) => URL.createObjectURL(image));
    setImageUrls(urls);

    // Cleanup function to revoke object URLs on unmount or when images change
  ...
  const handlePost = async () => {
    ...
    const postData = {
      ...
      userId,  // User ID from useUser hook
      privacy: "public",
      location: "New York",
    };

    try {
      await addPost(postData).unwrap();  // Assuming addPost is a mutation function
...
//     if (e.target.files) {
//       const selectedFiles = Array.from(e.target.files); // Convert FileList to an array
//       setImages(selectedFiles); // This now works because `images` is File[]
//     }
//   };
//   const uploadImageToImgBB = async (image:Image) => {
//     const formData = new FormData();
...
//     const data = await response.json();
//     console.log("this is from pc",data)
//     return data.data.url; // Return the image URL from ImgBB
//   };
