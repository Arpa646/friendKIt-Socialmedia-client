...
import { Post, Image } from "./types"; // Adjust the import path accordingly
...
const PostCard : React.FC<CardModalProps> = ({ post }) => {
  ...
  const [isLiked, ] = useState(false); // State to track if the post is liked
  ...
  const handleLike = async () => {
    ...
    try {
      // Send the mutation request to like the post
      await likePost({
        postId: post._id,
  ...
  return (
    <div className="post bg-background text-white p-4 rounded mb-4">
      ...
      <div className="post-header flex items-center">
        <img
          src={post.userId?.image } // Default image if none exists
          alt={ "User"}
        ...
        <div
          className={`grid gap-2 mt-4 ${
            post.images.length === 1
              ...
              ? "grid-cols-3"
              : "grid-cols-2" // Default to 2 columns for 4 or more images
          }`}
        >
      ...
      {isModalOpen && (
        <CommentModal post={post} userId={userId} onClose={closeModal} />
      )}
    </div>
  );
};
